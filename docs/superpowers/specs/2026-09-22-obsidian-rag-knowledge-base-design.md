...
- Markdown 感知的分块 + 混合检索（向量 + 关键词），宽召回
- 检索质量评估集（50~100 条查询 + Recall/MRR 指标）
...
| 查询端 | MCP server，只返回带出处的原文片段，宽召回交调用方选 |
...
| embedding 跳过窗口 | 少于 10 token 或超过 8000 token 的 chunk **不做 embedding**，只进全文索引 |

宽表重复表头这条是必需的：Excel 转出来的宽表，后半截 chunk 全是裸数字，没有任何检索价值。

**embedding 跳过窗口是实打实的省钱规则。** 过短的碎片（截图残留、单行注释）向量化没有意义；过大的表格块超出模型窗口，embed 出来的质量本来也不可信。这两类只进 `tsv` 全文索引即可 —— 而且用户查这类内容时用的本来就是精确词（表名、字段名、代码标识符），走关键词路反而更准。在百万 chunk 的目标规模下，这条能砍掉可观比例的 embedding 调用。
...
**Query embedding 缓存**：查询向量按 `md5(query)` 缓存，第一期用进程内 LRU，以后要 Redis 就换同一个接口的另一个实现。一次远程 embedding 往返是几百毫秒，而个人知识库的查询重复率不低 —— 这是全项目投入产出比最高的一处优化。

只缓存 **query** 的向量，不缓存 chunk 的：后者本来就只算一次，缓存只会白占内存。
...
query（先查 embedding 缓存）
 ...
 └─ 关键词路：websearch_to_tsquery → tsvector GIN，top 40
        ...
        每文档限量：同一文件最多 3 个 chunk，防霸榜
                      ↓
      宽召回 top N（默认 25）+ 出处 → 交给 Claude 自选
```
...
我们只做 2 路召回，排序能力天然弱于多路融合系统。而**排序恰恰是 RAG 最难做好的部分**：同领域的 kb 项目在 95 个案例上测出 `R@5 = 0.507` 但 `R@20 = 0.831` —— 12 个类别里有 11 个的正确答案**已经在候选集中，只是排不到前面**。他们的结论是「检索只管宽召回（不漏），选择和排序交给 Agent」。

**我们的调用方正是 Agent。** 分工应该是：后端负责不漏，Claude 负责选 —— 它有人类水平的判断力，而且已经配了 `read_note` 可以二次读取。如果只返回个位数的 chunk，等于用一个弱排序器替 Claude 做了它更擅长的决策，还把正确答案挡在门外。

默认 25 是折中：覆盖到 R@20 的召回水平，又不至于撑爆上下文。上限 50。
...
`query`（必填）、`limit`（默认 25，上限 50）、`tags`（可选）、`path_prefix`（可选，限定目录）。
...
| tool | 签名 | 用途 |
|---|---|---|
| `search_notes` | `(query, limit=25, tags?, path_prefix?)` | 主入口。宽召回带出处的片段，交 Claude 自选 |
...
系统要同时满足两件性质不同的事：**不坏**（正确性）和**搜得准**（检索质量）。验证手段完全不同，不要混在一起。
...
上面那张表保证系统**不坏**，这一节保证它**搜得准**。MVP 阶段就要建起来。

- **评估集**：50~100 条真实查询，从你自己的笔记反向构造。三种风格各覆盖一些 —— 关键词式（`SKIP LOCKED`）、语义式（「怎么防止任务重复执行」）、实体式（「张三那个方案」）
- **指标**：Recall@5、Recall@10、MRR
- **跑法**：每次改动检索逻辑（分块策略、RRF 参数、embedding 策略、路由权重）之后必须跑一遍

**不要一开始就定「R@5 必须 > 0.8」这种目标。** 先跑出基线，再根据基线决定往哪优化 —— 拍脑袋定的阈值只会逼着你调参数去凑数字，而不是真的改好系统。

为什么这条不能省：检索调优有大量**反直觉**的坑。同领域项目实测过一批看起来完全合理的改动，结果全是负收益 —— 放宽向量最低分阈值 **−2.5pp**、RRF 的 K 从 45 调到 20 **−2.3pp**、关掉时效性加权 **−2.9pp**、内部 top_k 从 10 调到 30 **−17.5pp**。没有评估集，你会把这些都当成改进合并进去。

他们还有一次更典型的教训：花了几轮修数据，指标纹丝不动（R@5 从 0.4393 到 0.4393），最后发现原因是融合公式里单条路径的 top-1 权重只有 `1/(45+1) ≈ 0.022` —— **单独优化一条路根本改变不了最终排名**。这种事只有评估集能告诉你。

他们把结论写成了一句话，值得直接抄过来：**「代码改动后必须跑 eval 验证，不能只看单元测试通过」。**
...
| PG 中文全文检索有天花板 | 关键词路依赖 `zhparser`。同领域项目在更大规模下最终把全文检索迁到了 Elasticsearch，PG `tsvector` 只留作 ES 同步窗口的兜底。第一期省掉了 ES 的运维成本和双写一致性代价，代价是这条路有上限 —— 触及天花板时需要重新评估 |
| 宽召回增加上下文占用 | 默认返回 25 个片段而非 8 个，调用方的上下文消耗更大。这是用 token 换召回的自觉取舍 |
...
- Elasticsearch 全文检索（若 PG `zhparser` 触及天花板）
- 评估集扩充并接入自动化回归
- 多用户注册与自助 token 管理
