...
**结论**：向量路**已修复**（5935 ms → 0.8 ms，走 HNSW）；关键词路的 GIN 索引在 RLS 下**结构性不可用**，已于 2026-09-23 拍板**接受现状**（见 §三）。
...
| **4. 移除 RLS** | `ALTER TABLE … DISABLE ROW LEVEL SECURITY`，租户隔离只留应用层 | **已否掉** —— 收益与方向 2 完全等价，风险半径却从"一条通路"放大到"全局且不可局部回退"，见下 |
...
### 决策（2026-09-23）：方向 1，接受现状

**拍板结论**：保留 RLS。关键词路接受"扫描本租户 chunks"的现状，不删策略、不引入
`BYPASSRLS` 函数、不做分区。

理由不是"8 ms 还能忍"，而是**收益与风险不对称**：

- **收益上限极低。** RLS 只挡住了关键词这一条通路：向量路在 RLS 下已命中 HNSW
  （0.81 ms），`list_notes` / `browse_documents` 走 btree 也不受影响。移除 RLS 的
  全部收益 = 关键词路 7.6 ms → 0.7 ms。而一次 `search_notes` 里还包含一次 embedding
  API 调用（网络往返，百毫秒级），6.9 ms 在端到端延迟里是噪声。
- **收益与方向 2 完全等价，风险不等价。** 方向 2 与"移除 RLS"都能让 `ix_chunks_tsv_gin`
  可用；差别在风险半径 —— 方向 2 只影响一条通路、租户 id 是显式参数、可写集成用例把
  "跨租户调用必须返回空"钉住；移除 RLS 影响所有表与所有未来代码，且无法局部回退。
- **当下就存在一个具体失效点。** `query_builder.all_enabled_repos_for_worker()` 是全模块
  唯一没有租户谓词的查询，它的安全网就是 RLS：跑错引擎时返回空集，而不是返回全部。
  删掉 RLS，这个失败模式立刻变成跨租户返回所有仓库。
- **会连带架空一批设计。** 双引擎（app / admin）、`settings` 里"两个 DSN 不能同角色"
  的启动校验，前提都是 RLS 生效；`test_tenant_isolation.py` 中 12 条跨租户红线用例会
  退化为空测试，举证只剩 owner 引擎那一条。
- **与既定路线冲突。** spec §13 把"多用户注册与自助 token 管理"列为演进方向，规模目标
  是"最终 >100 用户 / 百万级 chunk"。"第一期只跑通单用户链路"不等于"永久单租户"。

**若将来真要移除 RLS，唯一自洽的形态是彻底取消多租户** —— 连 `users` 的 user 维度、
`user_id` 列、租户谓词、双引擎、`app.user_id` 会话变量一起删，spec 与测试同步重写。
"保留多租户模型、只拆掉数据库那道锁"拿到了大改动的成本 + 降级红线，却没换来任何性能，
是最差组合。
...
### 附：一条尚未验证、预计也不成立的残留路径

§二 排除的是**直接改写**谓语形态，但还有一条没人试过：自定义一个标记为 `LEAKPROOF`
的函数 / 算子来包装 `tsv @@ tsquery`（`CREATE FUNCTION … LEAKPROOF` 需要 superuser，
而 alembic 正是以 superuser 跑的）。预期的结论是**不成立**，原因在两头：

- `LEAKPROOF` 标记只能让**谓语**过得了安全屏障检查；而 GIN 的算子匹配只认 opfamily 里
  注册过的 `@@`（及其同义 `@@@`）。自定义算子进不了 `tsvector_ops` 的 opfamily；
- 若改走"自定义函数 + 函数索引"，函数调用的形状又不会被当作索引条件匹配。

两条路互相冲突，所以"谓语 leakproof 了、索引却匹配不上"。留此记录以免重复试错。
**不做**这件事的成本也极低：真到触发条件时，方向 2 是更短的路径。
